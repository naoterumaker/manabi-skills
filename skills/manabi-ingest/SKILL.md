...
直後にレベル選択を提示。**初めてのユーザーはレベルの意味がわからない前提で、
「何が手に入るか」「誰向けか」「目安時間」を必ず添える**:
...
**BLOCKER（manifest競合の禁止）**: manifest.jsonへ書き込む処理を並列実行してはならない
（全体書き戻し方式のため、並列書き込みは互いの結果を消し合う）。並列化する場合は
「読み取り専用で並列→集約して1回書き込み」か、書き込みロックを設けること。

**処理計画（Step 1.5）には採用する並列方式と並列度を必ず明記し、承認を得ること。**
実行者の独自判断で方式を変える場合も、変更内容を提示してから実行する。
